...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            # ---> INJEÇÃO DE LOG: SUCESSO DE AUTENTICAÇÃO <---
            current_app.logger.info(f"AUDITORIA: Usuario {form.username.data} autenticado com sucesso no sistema.")
            
            task_form = TaskForm()
            flash('Login Successfull', 'success')
            return redirect(url_for('all_tasks'))
        else:
            # ---> INJEÇÃO DE LOG: FALHA/VIOLAÇÃO DE SEGURANÇA <---
            current_app.logger.warning(f"VIOLACAO/FALHA DE SEGURANCA: Tentativa de login invalida para o usuario: {form.username.data}")
